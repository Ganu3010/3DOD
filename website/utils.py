...
def to_pcd(ip_file):
    ...
    points[:, -3:] /= 255 if np.amax(points) >= 255 else 1
...
def preprocess(filepath, model, dataset):
    ...
    if model == 'spformer' and dataset == 'scannetv2':
        ...
        save_path = os.path.join(filepath.replace(
            'input', 'preprocessed'), filename.split('.')[0] + '.pth')
...
def get_bounding_boxes(output):
    ...
    with open(os.path.join(output, 'pred_instance', '.txt'), 'r') as f:
        masks = f.readlines()   
...
def get_coords_color(output):
    ...
    '''
    app = current_app
    file = os.path.join(app.config['ROOT_FOLDER'], 'static', 'preprocessed', os.path.split(
        output)[-1].split('.')[0] + '.pth')
